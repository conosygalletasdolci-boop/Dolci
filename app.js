...
function saveCart(){ localStorage.setItem("dolciCart", JSON.stringify(cart)); }
...
function cartCount(){ return cartEntries().reduce((s,x)=>s+x.qty,0); }
function cartTotal(){ return cartEntries().reduce((s,x)=>s+x.product.price*x.qty,0); }
...
function renderHomeCategories(){
  $("homeCategories").innerHTML = Object.entries(categories).map(([key,c]) => `
    <button class="home-category" data-category="${key}">
...
function renderCategoryNav(){
  $("categoryNav").innerHTML = Object.entries(categories).map(([key,c])=>`
    <button class="${key===selectedCategory?"active":""}" data-category="${key}">
      <img src="${c.image}" alt="${c.name}">
      <span>${c.name}</span>
    </button>
  `).join("");
...
function renderCart(){
  ...
  $("cartItems").innerHTML = entries.length ? entries.map(x=>`
    <div class="cart-line">
      ...
      <div>
        ...
        <div class="mini-qty">
          <button data-cart-minus="${x.product.id}">−</button>
          <span>${x.qty}</span>
          <button data-cart-plus="${x.product.id}">+</button>
        </div>
      </div>
      <div class="line-total">${money(x.product.price*x.qty)}</div>
    </div>
  `).join("") : `<p class="modal-subtitle">Aún no has agregado productos.</p>`;

  $("cartTotal").textContent=money(cartTotal());

  document.querySelectorAll("[data-cart-plus]").forEach(b=>b.addEventListener("click",()=>{
    changeQty(b.dataset.cartPlus,1); renderCart();
  }));
  document.querySelectorAll("[data-cart-minus]").forEach(b=>b.addEventListener("click",()=>{
    changeQty(b.dataset.cartMinus,-1); renderCart();
  }));
}
...
function formatDate(){
  return new Intl.DateTimeFormat("es-CO",{dateStyle:"medium",timeStyle:"short"}).format(new Date());
}
...
function createOrder(data){
  const order={
    ...
    notes:data.notes,
    items:cartEntries().map(x=>({...x.product,qty:x.qty,lineTotal:x.product.price*x.qty})),
    total:cartTotal()
  };
  lastOrder=order;
...
function sendWhatsApp(){
  if(!lastOrder) return;
  const url=`https://wa.me/${BUSINESS_WHATSAPP}?text=${encodeURIComponent(whatsappText(lastOrder))}`;
...
async function shareInvoice(){
  ...
  if(!navigator.share){
    alert("Tu navegador no permite compartir la factura directamente. Usa 'Imprimir / Guardar PDF'.");
    return;
  }

  // Convierte la factura HTML en una imagen sencilla mediante SVG embebido.
  const html = invoice.innerHTML.replace(/</g,"&lt;").replace(/>/g,"&gt;");
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1000" height="1500">
    <foreignObject width="100%" height="100%">
      <div xmlns="http://www.w3.org/1999/xhtml" style="font-family:Arial;background:white;padding:35px;color:#111">${html}</div>
    </foreignObject>
  </svg>`;
  const blob=new Blob([svg],{type:"image/svg+xml"});
  const file=new File([blob],`factura-${lastOrder.number.replace("#","")}.svg`,{type:"image/svg+xml"});

  try{
    if(navigator.canShare && navigator.canShare({files:[file]})){
      await navigator.share({title:`Factura ${lastOrder.number} - Dolci`,text:whatsappText(lastOrder),files:[file]});
    }else{
      await navigator.share({title:`Factura ${lastOrder.number} - Dolci`,text:whatsappText(lastOrder)});
    }
  }catch(e){}
}
...
document.querySelector('[data-nav="home"]').addEventListener("click",()=>openView("homeView"));
